...
public class MazeGenerator : MonoBehaviour
{
    ...
    public PlayerStats playerStats; // Assign this in the Unity Inspector
    ...
    void Start()
    {
        playerStats.PrepareForNextLevel(); // Resets health every time game starts
    ...
    private void SpawnEnemies()
    {
        ...
        int enemyCount = Mathf.Clamp(playerStats.level, 2, maxEnemies);
        List<Vector2> validPositions = new List<Vector2>();
        ...
        for (int x = 1; x < width - 1; x++)
        {
            for (int y = 1; y < height - 1; y++)
            {
                if (Maze[x, y] == 0) validPositions.Add(new Vector2(x, y));
            }
        }
        ...
        for (int i = 0; i < enemyCount; i++)
        {
            ...
            Vector2 spawnPos = validPositions[rnd.Next(validPositions.Count)];
            validPositions.Remove(spawnPos);
            Vector3 worldPos = floorTilemap.CellToWorld(new Vector3Int((int)spawnPos.x, (int)spawnPos.y, 0)) + new Vector3(0.5f, 0.5f, 0);
            Instantiate(spawnEnemy, worldPos, Quaternion.identity);
        }
    }
